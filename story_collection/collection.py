...
def extract_urls_from_source(source):
    try:
        ...
        openai_result = process_content_with_openai(content, setup_prompt, answer_format, JSON_SCHEMAS['url_collection'])
...
def extract_stories_from_urls(urls, source):
    ...
    for url in urls:
        ...
        # Process content with OpenAI
        setup_prompt = "You are a helpful assistant designed to output JSON. " \
                       "Your task is to extract the news story from the given content, which includes " \
                       "the extracted text content, meta properties and images of a HTML page"
        answer_format = "Please return the story in a JSON format, extracting the following properties from the HTML content: " \
                        ...
                        "If the content contains a summary, extract it." \
                        "If it doesn't contain a summary, write a summary of maximum 30 words in length based on the story." \
                        "If the extracted summary exceeds 30 words, summarize it to max. 30 words in length. "
        openai_result = process_content_with_openai(setup_prompt, story_content, answer_format, JSON_SCHEMAS['story_collection'])

        # Check if openai_result is not None before parsing it
        if openai_result is not None:
            # Parse and save the story to the database
            try:
                story_data = json.loads(openai_result)
